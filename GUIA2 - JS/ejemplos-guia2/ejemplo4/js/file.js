...
//Solicitar el valor para el radio de circulo
...
document.write("</div>");
document.write("<p>");
//Recorrer propiedades del objeto Math usando la instruccion with
with(Math){
    //Área de un circulo de radio "radio"
    area = PI * radio*radio;
    //Valor del lado horizontal defionido por el radio
    coorx = abs(radio*cos(PI/4));
    //Valor del lado vertical definido por el radio
    coory = abs(radio * sin(PI / 4));
    pericir = 2 * PI *radio;
    perirec = 2*coorx + 2*coory;
    //Invocar la propiedad write del obejto documento with
    with(document){
